...
import am4geodataUsaAlbersLow from '@amcharts/amcharts4-geodata/usaAlbersLow';
...
class StatiChart7 extends React.Component {
  componentDidMount() {
    /* Chart code */
    // Themes begin
    am4core.useTheme(am4themesAnimated);
    ...
    const mainContainer = am4core.create('chartdiv7', am4core.Container);
    ...
    const maleChart = mainContainer.createChild(am4charts.XYChart);
    ...
    const femaleChart = mainContainer.createChild(am4charts.XYChart);
    ...
    const mapChart = mainContainer.createChild(am4maps.MapChart);
    mapChart.projection = new am4maps.projections.Mercator();
    mapChart.geodata = am4geodataUsaAlbersLow;
    ...
    polygonTemplate.events.on('hit', (event) => {
      const { id } = event.target.dataItem.dataContext;
    ...
    mapChart.seriesContainer.background.events.on('over', (event) => {
      // eslint-disable-next-line no-use-before-define
    ...
    function showState(id, stateName, polygon) {
      if (selectedStateId !== id) {
        // eslint-disable-next-line no-use-before-define
        const newData = stateData[id];

        if (selectedPolygon) {
          selectedPolygon.isActive = false;
        }

        for (let i = 0; i < femaleChart.data.length; i += 1) {
    ...
    mainContainer.exporting.menu = new am4core.ExportMenu();
  }
